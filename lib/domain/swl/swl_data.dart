...
class SwlData {
  ...
  SwlData({
    ...
    _count = count;
  ///
  List<double> _parseStrringList(List<String> strings) {
    return strings.map((e) {
      try {
        final v = double.parse(e);
        return v;
      } catch (error) {
        log(_debug, 'Ошибка в методе $runtimeType._parseStrringList() значение: $e \nошибка: $error'); 
        return 0.0;       
      }
    }).toList();
  }
  ...
  Future<List<double>> _loadAsset(String assetName) {
    return rootBundle.loadString(assetName)
      .then((value) {
        final doubleList = _parseStrringList(
